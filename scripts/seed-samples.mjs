...
const ROSTER = [
  ...
  { ext_id: "bal-1", name: "Balance · tech mgr (referral)", raw: { level: "manager", referral: true, tech: true, sourcing_date: "2024-12-01", offer_date: "2025-01-10", join_date: "2025-02-15", total_ctc: 100000, ctc_ccy: "USD" } },
  { ext_id: "bal-2", name: "Balance · NON-tech mgr (non-ref)", raw: { level: "manager", referral: false, tech: false, sourcing_date: "2024-12-01", offer_date: "2025-01-10", join_date: "2025-02-20", total_ctc: 100000, ctc_ccy: "USD" } },
];
const INPUTS = [
...
// FULL rate matrix: every level × referral × tech combination gets its own %.
// referral REDUCES the rate (adjRef); non-tech is lower than tech (adjNonTech).
// keys must include "tech" so the engine actually varies the % by tech.
const TA_KEYS = ["level", "referral", "tech"];
const taMatrix = (levels, adjRef = 2, adjNonTech = 1) => {
  const rows = [];
  for (const [level, base] of Object.entries(levels))
    for (const referral of [false, true])
      for (const tech of [true, false])
        rows.push({ level, referral, tech, pct: Math.round((base - (referral ? adjRef : 0) - (tech ? 0 : adjNonTech)) * 100) / 100 });
  return rows;
};
const taRows = () => taMatrix({ non_leadership: 8.5, manager: 9.5, director: 11 }, 2, 1);
...
const SAMPLES = [
  { code: "KENVUE-GCC", name: "Kenvue GCC",
    ...
    heads: [
      ...
      { code: "ta", kind: "one_time_split", rate_table: { keys: TA_KEYS, rows: taRows() }, milestones: m3({ tech: 400, nontech: 300 }, { tech: 600, nontech: 300 }), clause_ref: "§3" },
    ] },
  { code: "NORTHWIND-GCC", name: "Northwind GCC",
    ...
    heads: [
      ...
      { code: "recruitment", kind: "one_time_split", rate_table: { keys: TA_KEYS, rows: taMatrix({ non_leadership: 9, manager: 11, director: 14 }, 3, 1.5) }, milestones: m3({ tech: 600, nontech: 450 }, { tech: 800, nontech: 600 }), clause_ref: "§3" },
    ] },
  ...
  { code: "HELIO-GCC", name: "Helio GCC",
    ...
    heads: [
      { code: "recruitment", kind: "one_time_split", rate_table: { keys: TA_KEYS, rows: taMatrix({ non_leadership: 7, manager: 8, director: 10 }, 2, 1) }, milestones: m3({ tech: 400, nontech: 300 }, { tech: 500, nontech: 350 }), clause_ref: "§3" },
