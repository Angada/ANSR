...
const DEFAULT_CONFIG = {
  ...
  pipelines: {
    ...
    "contra-archetype": { id: "contra-archetype", product: "Contra", name: "Archetype Maker", kind: "llm",
      ...
      prompt: "You are a contract-review architect. From the sample contract, propose the important SECTIONS a reviewer must check for this TYPE of contract — thematic areas, NOT a clause-by-clause list. Return STRICT JSON only: {\"name\":\"suggested archetype name\",\"sections\":[{\"key\":\"snake_case\",\"label\":\"Human label\",\"what_to_check\":\"what a reviewer verifies here\",\"required\":true|false}]}. THE KEY MUST COME FROM THIS FIXED VOCABULARY, exactly as spelled: particulars, parties, commercial_terms, payment_terms, liability, indemnity, warranties, term_termination, confidentiality, data_protection, ip, compliance, insurance, service_levels, change_control, subcontracting, governing_law, dispute_resolution, force_majeure, assignment, notices. Choose the closest key for each theme; use the label for the contract\u2019s own wording. Only if a genuinely important theme fits NONE of these, invent one snake_case key. Two contracts of the same type must produce the SAME keys — a key is an identity, not a description. Order them the way a reviewer reads them. Mark the genuinely important ones required." },
