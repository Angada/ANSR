...
4. **Calc engine** — deterministic interpreter runs the rule book over the placement ledger → facts + a **trace per number**; clean rows compute, bad rows **quarantine** (partial-compute). Cumulative ledger → cross-period roll-forwards. Live **FX** (cache + manual override). → `server/engine/compute.js`, `operators.js`, `run.js`, `fx.js`.
